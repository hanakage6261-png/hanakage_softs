...
momonGA_systems/
  ...
  momonGA_searching/
    ...
    momonGA_metadataonly_searching.py
    momonGA_null_ID_researching.py
  momonGA_utilize/
    random_select.py
...
対象: `momonGA_downloader/momonGA_downloader.py`
...
対象: `momonGA_searching/momonGA_searching.py`

- `mo0` からIDを順に確認。
- 再開位置は `momonGA_searching_state.json` の `next_id`。
- DBの最大IDは再開位置に使用しない。
...
- `momonGA_searching_state.json` の `next_id` を追い越さない。
...
将来 `momonGA_metadata_store.py` に設定ファイル読込を追加し、DB絶対パスを指定できるようにする。
...
```powershell
python -m py_compile .\momonGA_metadata_store.py
python -m py_compile .\momonGA_downloader\momonGA_downloader.py
python -m py_compile .\momonGA_searching\momonGA_searching.py
python -m py_compile .\momonGA_searching\momonGA_metadataonly_searching.py
```
...
- `momonGA_searching_state.json` の値をDB最大IDから作り直さない。
